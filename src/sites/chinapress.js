import { chinapressConfig as config } from './chinapress.config.js';
...
function assertUsableResponse(response, url) {
  ...
  if (crawlPolicy.stopStatuses.includes(status)) {
    const error = new Error(`China Press returned HTTP ${status} for ${url}`);
    error.status = status;
    throw error;
  }
}

export const chinapressAdapter = {
  ...config,
  today: malaysiaDate,
  ...
  async collectLinks(page, today) {
    ...
    return links.filter(({ href }) => {
      ...
      if (parsed.hostname !== 'www.chinapress.com.my' || dateFromArticleUrl(href) !== today || seen.has(href)) return false;
      seen.add(href);
      return true;
      ...
      const metadata = await page.evaluate(({ selectors, ignoredParagraphPrefixes }) => {
        ...
        const meta = (list) => {
          ...
          return selectorsToTry.map((selector) => {
            ...
            return element?.content || element?.getAttribute('data-pdatetime') || element?.innerText || '';
          }).find(Boolean) || '';
        };
